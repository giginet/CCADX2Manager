...
/* バージョン情報 */
/* Version Number */
#define CRIATOMEX_VERSION				(0x02010100)
#define CRIATOMEX_VER_NUM				"2.01.01"
...
{\
	...
	(p_config)->max_aisac_auto_modulations = 8;\
...
typedef struct CriAtomExConfigTag {
	...
	*/
	...
	/*JP
		...
		\attention
		本パラメータはAtom Ver.2.00.00以降のライブラリで使用予定です。<br>
		現状、本パラメータに設定した値は使用されません。<br>
	*/
	...
	*/
	...
	/*JP
		...
		Atomライブラリは、初期化時に max_sequences で指定された数分
	...
	*/
	...
	/*JP
		...
		Atomライブラリは、初期化時に max_tracks で指定された数分
	...
	*/
	...
	/*JP
		...
		Atomライブラリは、初期化時に max_track_items で指定された数分
	...
	*/
	...
	/*JP
		\brief 最大AISACオートモジュレーション数
		\par 説明:
		アプリケーションで同時再生するAISACオートモジュレーションの総数です。<br>
		Atomライブラリは、初期化時に max_aisac_auto_modulations で指定された数分
		のAISACオートモジュレーションを作成できるリソースを確保します。<br>
		\attention
		max_aisac_auto_modulations で指定された数以上のAISACオートモジュレーションを
		動作させることはできません。<br>
		AISACオートモジュレーションは、AISACのコントロールタイプにオートモジュレーションを
		選択した場合に動作します。<br>
		エラーコールバックが発生した場合、この値を大きくしてください。<br>
	*/
	...
	*/
	...
	/*JP
		...
		ここで設定し、初期化時に確保したフェーダーはMusicTransitionデータ再生時にライブライ内部で使用します。<br>
	*/
...
typedef enum CriAtomExAcbCueTypeTag {
	...
	CRIATOMEXACB_CUE_TYPE_MUSIC_TRANSITION,		/*JP< トラックトランジション再生（セレクタを参照して再生トラックを切り替える）	*/
	/* enum size is 4bytes */
...
typedef struct CriAtomExCuePos3dInfoTag {
	...
	CriUint16 angle_aisac_control;		/*JP< 角度AISACコントロール		*/
...
/*JP
 * \brief AISACオートモジュレーションの使用状況の取得
 * \ingroup ATOMEXLIB_GLOBAL
 * \param[out]	cur_num		現在使用中のAISACオートモジュレーション数
 * \param[out]	limit		利用可能なAISACオートモジュレーションの最大数
 * \par 説明:
 * 現在使用中のAISACオートモジュレーションの数、および利用可能な
 * 最大AISACオートモジュレーション数（＝ライブラリ初期化時に指定した CriAtomExConfig::max_aisac_auto_modulations の数）
 * を取得します。<br>
 */
 ...
 * \param[in]	work_size	ワーク領域サイズ
 * \par 説明:
 * プレーヤにフェーダをアタッチ（取り付け）します。<br>
 * 本関数でフェーダをアタッチしたプレーヤは、以降音声再生開始毎
 * （ ::criAtomExPlayer_Start 関数実行毎）に以下の制御を行うようになります。<br>
 * - 現在再生中の音声をフェードアウト。
 * - 新規に再生を開始する音声をフェードイン。
 * .
 * <br>
 ...
 * ::criAtomExPlayer_Stop 関数が実行された時点で即座に停止されます。<br>
 * <br>
 * 引数 config の情報は、関数内でのみ参照されます。<br>
 * 関数を抜けた後は参照されませんので、関数実行後に config の領域を解放しても
 * 問題ありません。
